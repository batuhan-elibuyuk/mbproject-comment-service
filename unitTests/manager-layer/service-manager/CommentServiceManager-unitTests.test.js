...
describe("CommentServiceManager", () => {
  let CommentServiceManager;
  let ApiManagerMock;
  ...
  beforeEach(() => {
    ...
    CommentServiceManager = proxyquire(
      "../../../src/manager-layer/service-manager/CommentServiceManager",
      {
  ...
  describe("userHasRole()", () => {
    it("should return true if userHasRole returns true", () => {
      ...
      const manager = new CommentServiceManager(req, {});
      expect(manager.userHasRole("admin")).to.be.true;
    });

    it("should return false if no auth", () => {
      const manager = new CommentServiceManager({}, {});
