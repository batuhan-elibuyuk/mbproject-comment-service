...
console.log('Starting mbproject-comment-service service...');
...
const {initService,getPublicKey,setCurrentKeyId,startRepairElastic} = require("utils");
...
const start = async () => {
  process.title = "node-mbproject-comment-service";
  ...
  try {
    ...
    await setRedisData("mbproject-comment-service:start",today.toUTCString());
    console.log('Service start datetime is written to Redis key mbproject-comment-service:start',today.toUTCString());
  } catch(err) {
    console.log('Redis test failed:',err);
    hexaLogger.insertError("RedisConnectionFailedError",{  }, "index.js->start",err);
  }


  try {
    await hexaLogger.updateLoggerMappings(); 
    await hexaLogger.insertInfo("LoadingService", {time:today.toUTCString()}, "index.js->start");
  } catch(err) {
  ...
  try {
    ...
    hexaLogger.insertError("ElasticIndexMappingUpdateError",{ }, "index.js->start", err);
  }
  try {
    ...
    hexaLogger.insertError("ElasticIndexRepairError",{ }, "index.js->start", err);
  }

    const expressApp = require("./express-app");
    const servicePort = process.env.HTTP_PORT ?? 3000;
    expressServer = expressApp.listen(servicePort);
    console.log("mbproject-comment-service is listening HTTP/REST port " + servicePort.toString());
    await hexaLogger.insertInfo("ListeningHttpPort",{port:servicePort},"index.js->start", {date:today.toUTCString()});
    
    grpcServer.bindAsync("0.0.0.0:50000", grpc.ServerCredentials.createInsecure(), (error, port) => {
      if (!error) {
        grpcServer.start();
    ...
    if  (process.env.NODE_ENV !== "dev") {
      try {
        ...
        hexaLogger.insertError("KafkaConnectionFailedError",{},"index.js->",err);
      }
    }
    ...
    try {
      ...
      if (processActive) {
        ...
        const insertCloseLogPromise =  hexaLogger.insertInfo(
          'ClosingApp', {active_handles:process.getActiveResourcesInfo().length},
          "index.js->close"
        );
  ...
  /* starting the server mbproject-comment-service .... */
  start();
